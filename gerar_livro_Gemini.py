...
OUTPUT_DOCX = "Livro_Final_Formatado_Gemini.docx" # Changed output name
...
# MAX_CHUNK_TOKENS: Maximum number of tokens for the INPUT chunk sent to the API.
# Gemini 1.5 Flash has a 1M token context window. 10000 tokens is a very safe chunk size.
MAX_CHUNK_TOKENS = 10000

# MAX_OUTPUT_TOKENS: Maximum number of tokens the model will GENERATE in its response for each chunk.
# Gemini 1.5 Flash supports up to 8192 output tokens according to docs.
# Keeping 4096 is safe and works across models, but you could try up to 8192 if needed.
...
try:
    ...
    # You can check available models with genai.list_models() if needed
...
# We will use a simple character count approximation for chunking
# because tiktoken is for OpenAI models and calling Gemini's count_tokens
# repeatedly during chunking might be slow or hit rate limits.
def count_tokens_approx(text):
    """Approximate token count using character count."""
    if not text:
        return 0
    # Rough approximation: 1 token ≈ 3.5-4 chars for Latin scripts
    return len(text) // 4
...
def create_chunks(text, max_tokens):
    ...
    Returns a list of chunks, each containing text content <= max_tokens (approximate).
    Attempts to avoid splitting mid-paragraph or mid-sentence if possible.
    Uses approximate token count for chunking logic.
    """
    logger.info(f"Starting chunk creation with max_tokens per chunk (approx): {max_tokens}")
    chunks = []
    current_chunk = ""
    ...
    for i, paragraph in enumerate(paragraphs):
        ...
        # Use approximate token count for chunking logic
        paragraph_tokens = count_tokens_approx(paragraph)

        # Check if adding this paragraph would exceed the limit
        # Add approx tokens for the potential "\n\n" if it's not the first paragraph in the chunk (approx 2 chars = 0-1 token)
        tokens_with_separator = paragraph_tokens + (1 if current_chunk else 0) # Add 1 token approx for separator

        # If adding this paragraph pushes the current chunk over the limit, save current chunk and start a new one
        # Ensure the paragraph itself isn't already too big (should be handled by splitting below if needed)
        if current_chunk_tokens + tokens_with_separator > max_tokens and current_chunk_tokens > 0:
            chunks.append(current_chunk.strip())
            logger.debug(f"Saved chunk {len(chunks)}. Approx Tokens: {current_chunk_tokens}")
            current_chunk = ""
            current_chunk_tokens = 0

        # Handle cases where a single paragraph is larger than max_tokens (using approx count)
        if paragraph_tokens > max_tokens:
            logger.warning(
                f"Paragraph {i+1} exceeds chunk token limit (approx {paragraph_tokens} > {max_tokens}). Attempting splitting by sentences.")
            ...
            for sentence in sentences:
                ...
                if temp_tokens + sentence_tokens <= max_tokens:
                    ...
                    if temp_chunk.strip():
                        ...
                        logger.debug(f"Saved split sentence chunk {len(chunks)}. Approx Tokens: {temp_tokens}")
                    temp_chunk = sentence + " " # Start new temp chunk with the current sentence
                    temp_tokens = sentence_tokens
            if temp_chunk.strip():
                chunks.append(temp_chunk.strip())
                logger.debug(f"Saved final split sentence chunk {len(chunks)}. Approx Tokens: {temp_tokens}")
            ...
            if is_chapter_start and current_chunk.strip() and i > 0:
                chunks.append(current_chunk.strip())
                logger.debug(f"Saved chunk {len(chunks)} at chapter start. Approx Tokens: {current_chunk_tokens}")
    ...
    if current_chunk.strip():
        chunks.append(current_chunk.strip())
        logger.debug(f"Saved final chunk {len(chunks)}. Approx Tokens: {current_chunk_tokens}")
...
# Pass the gemini_model object to the formatting function
def format_with_ai(gemini_model, chunk, is_first_chunk=False):
    """Process a chunk of text with the Gemini API and format it."""
    # Using the model's count_tokens method for a more accurate count before sending
    try:
        input_token_count = gemini_model.count_tokens(chunk_prompt).total_tokens
        logger.debug(f"Calling API for chunk (first: {is_first_chunk}). Input tokens: {input_token_count}")
        # Check if input tokens exceed model limit before calling
        # Gemini 1.5 Flash has 1M+ input tokens limit, so unlikely to exceed with 10k chunk
        # but good practice for other models/chunk sizes
        model_input_limit = 1000000 # Example limit for 1.5 Flash
        if input_token_count > model_input_limit:
             logger.error(f"Input token count {input_token_count} exceeds model limit {model_input_limit}.")
             # Decide how to handle this: skip, split further? For now, let API handle it.

    except Exception as e:
         logger.warning(f"Could not count tokens for chunk input: {e}")
         logger.debug(f"Calling API for chunk (first: {is_first_chunk}).") # Cannot easily count tokens before sending with Gemini SDK like with tiktoken
        ...
        try:
            ...
            response = gemini_model.generate_content(
                ...
                generation_config=genai.GenerationConfig(
                    ...
                    max_output_tokens=MAX_OUTPUT_TOKENS # Use the corrected constant
                )
                # Gemini models also have safety settings by default, which can be configured
            ...
            # It's commonly available as response.text for text-only outputs
            formatted_text = response.text
            logger.debug(f"Successfully processed chunk.")
            # Can count output tokens with model.count_tokens after getting response
            # output_token_count = gemini_model.count_tokens(formatted_text).total_tokens
            ...
            # Check for specific errors like BlockedPromptException if needed
...
def main():
    logger.info("Iniciando processamento do livro com Gemini.")
    ...
    logger.info(f"Fragmentando o texto em chunks de ~{MAX_CHUNK_TOKENS} tokens (aproximado)...")
    ...
    logger.info("Processando fragmentos com Gemini e inserindo no documento...")
    ...
    for i, chunk in enumerate(tqdm(text_chunks, desc="Processando fragmentos", unit="chunk")):
        ...
        # Process this chunk using the AI (Pass the gemini_model object)
