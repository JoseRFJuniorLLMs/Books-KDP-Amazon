...
# -- Nomes Base dos Arquivos de Saída (Tradução) --
                ...
                for sentence_num, sentence in enumerate(sentences):
                    ...
                    if current_sub_chunk_tokens + tokens_with_sub_separator <= max_tokens:
                        ...
                        if sentence_tokens > max_tokens:
                            ...
                            logger.warning(f"{log_prefix} -> Sentença/Linha {sentence_num+1} ({sentence_tokens} tk) excede limite. Adicionada como chunk individual.")
                            current_sub_chunk = ""; current_sub_chunk_tokens = 0
                        else:
                            current_sub_chunk = sentence; current_sub_chunk_tokens = sentence_tokens
                if current_sub_chunk: chunks.append(current_sub_chunk); sub_chunks_added_count += 1
                if sub_chunks_added_count == 0:
                     logger.warning(f"{log_prefix} Parágrafo {i+1} não subdividido. Adicionando original.")
                     chunks.append(paragraph_text)
                current_chunk = ""; current_chunk_tokens = 0 # Reseta após subdivisão
        ...
        try:
            ...
            try:
                if hasattr(response, 'usage_metadata'):
                    ...
                    total_tokens = usage.total_token_count if hasattr(usage, 'total_token_count') else (prompt_tokens + output_tokens) # Calcula se não existir
                    # logger.debug(f"{log_prefix} API OK ({latency:.2f}s). Tokens: P{prompt_tokens} + O{output_tokens} = T{total_tokens}") # Verbose
            ...
            if not response.candidates:
                 ...
                 try:
                    ...
                    if finish_reason == "STOP" and result_text:
                        return (result_text, latency, prompt_tokens, output_tokens, total_tokens) # SUCESSO
                         ...
                         if finish_reason != "STOP":
                             ...
                             if finish_reason == "SAFETY" and candidate.safety_ratings: logger.warning(f" -> Safety Ratings: {candidate.safety_ratings}")
                         if not result_text:
...
def run_final_txt_generation(marked_text_content, output_notes_path, output_numbered_txt_path, author_name, book_name):
    ...
    """
    log_prefix = f"[{author_name}/{book_name}]"
    logger.info(f"{log_prefix} --- Iniciando Passo 3: Geração TXTs Finais ---")
    ...
    try:
        ...
        logger.info(f"{log_prefix} TXT numerado [{footnote_counter-1}] salvo ({len(final_numbered_text)} chars).")
...
def send_completion_email(sender_email, sender_password, recipient_email, smtp_server, smtp_port,
    ...
    if not sender_email or not sender_password or not recipient_email:
        logger.warning("E-mail config incompleta no .env ou código. Notificação desativada.")
        return
...
--------------------------------------------------
- Livros Corrigidos (nesta execução): {processed_correction}
...
"""
    if processed_correction > 0:
        body += f"- Tempo Total Correção (livros OK): {total_correction_time_secs:.2f} seg\n"
        body += f"- Tempo Médio por Correção: {avg_correction_time_secs:.2f} seg\n"
        # Adiciona stats da API para Correção (Passo 1)
        body += f"- Tempo Total API (Passo 1 Correção): {total_correction_latency_secs:.2f} seg\n"
        body += f"- Tokens Totais API (Passo 1 Correção): {total_correction_tokens}\n"
        # Adiciona stats da API para Notas (Passo 2)
        body += f"- Tempo Total API (Passo 2 Notas): {total_footnote_latency_secs:.2f} seg\n"
...
--------------------------------------------------
- Livros Traduzidos (nesta execução): {processed_translation}
...
"""
    if processed_translation > 0:
        body += f"- Tempo Total Tradução (livros OK): {total_translation_time_secs:.2f} seg\n"
...
# --- FUNÇÃO PRINCIPAL (main - MODIFICADA para acumular e passar stats) ---
def main():
    start_time_main = time.time()
    logger.info("========================================================")
    logger.info(f"Iniciando Processador (Correção + Tradução) - {time.strftime('%Y-%m-%d %H:%M:%S')}")
    # ... (logs de config iniciais) ...
    logger.info("========================================================")
    ...
    # << Acumuladores GERAIS para stats da API e tempos >>
    ...
    correction_times = [] # Tempos de processamento da correção por livro
    translation_times = [] # Tempos de processamento da tradução por livro
    ...
    for author_name in author_folders:
        ...
        # ... (busca e filtro de TXT - inalterado) ...
        try: # Find and filter TXT files
            input_txt_files = sorted(glob.glob(os.path.join(author_input_dir, "*.txt")))
            input_txt_files = [f for f in input_txt_files if not (os.path.basename(f).endswith(FINAL_NUMBERED_TXT_BASENAME) or os.path.basename(f).endswith(NOTES_TXT_FILE_BASENAME) or os.path.basename(f).startswith("backup_"))]
        except Exception as e: logger.error(f"[{author_name}] Erro ao listar/filtrar .txt: {e}"); continue
        if not input_txt_files: logger.warning(f"[{author_name}] Nenhum .txt de entrada encontrado."); continue
        logger.info(f"[{author_name}] {len(input_txt_files)} arquivos .txt encontrados.")
        ...
        for input_txt_path in input_txt_files:
            ...
            author_output_docx_dir = os.path.join(BASE_OUTPUT_DOCX_DIR, author_name) # Garante definição
            ...
            if file_identifier in processed_files_set:
                ...
                try: os.makedirs(author_output_docx_dir, exist_ok=True); os.makedirs(author_output_txt_dir, exist_ok=True)
                except Exception as e_mkdir: logger.error(f"{log_prefix_book} ERRO ao criar dirs: {e_mkdir}"); total_books_failed_correction += 1; continue
                ...
                try:
                    # PASSO 1: CORREÇÃO DOCX (Captura stats)
                    pass1_success, corrected_text_content, lat1, p1, o1, t1 = run_correction_pass(
                        gemini_model, input_txt_path, TEMPLATE_DOCX, output_docx_path, author_name, base_book_name
                    )
                    # Acumula stats do passo 1
                    ...
                    if not pass1_success or corrected_text_content is None:
                        ...
                        # PASSO 2: IDENTIFICAÇÃO DE NOTAS (Captura stats)
                        pass2_success, marked_text_content, lat2, p2, o2, t2 = run_footnote_id_pass(
                            gemini_model, corrected_text_content, author_name, base_book_name
                        )
                        # Acumula stats do passo 2
                        ...
                        if not pass2_success or marked_text_content is None:
                            ...
                            pass3_success = run_final_txt_generation(
                                marked_text_content, output_notes_path, output_numbered_txt_path, author_name, base_book_name
                            )
                            if not pass3_success: logger.error(f"{log_prefix_book} Passo 3 (Geração TXTs Finais) FALHOU."); all_correction_steps_successful = False
                except Exception as e_corr_steps:
                     logger.error(f"{log_prefix_book} Erro inesperado nos passos de CORREÇÃO: {e_corr_steps}"); logger.error(traceback.format_exc()); all_correction_steps_successful = False
            ...
            if correction_successful_this_run:
                ...
                if output_docx_path is None or not os.path.exists(output_docx_path):
                     logger.warning(f"{log_prefix_book} Input DOCX para tradução não encontrado. Pulando."); total_translation_failed += 1
                elif file_identifier in translated_files_set:
                    logger.info(f"{log_prefix_book} TRADUÇÃO já feita. Pulando."); total_translation_skipped += 1
                else:
                    ...
                    if not os.path.exists(PATH_TO_TRANSLATOR_SCRIPT):
                         logger.error(f"{log_prefix_book} ERRO CRÍTICO: Script tradutor '{PATH_TO_TRANSLATOR_SCRIPT}' não encontrado."); total_translation_failed += 1
                    else:
                        try:
                            ...
                            if result.returncode == 0:
                             ...
                             logger.error(f"{log_prefix_book} Erro CRÍTICO subprocesso tradução: {e_translate_sub}"); logger.error(traceback.format_exc()); total_translation_failed += 1
            # --- Fim Tradução ---
    ...
    if correction_times:
        logger.info(f"Tempo Total Correção (livros OK): {total_corr_time_ok:.2f} seg")
    ...
    if translation_times:
        logger.info(f"Tempo Total Tradução (livros OK): {total_trans_time_ok:.2f} seg")
    ...
    # === Envio de E-mail FINAL (com mais stats) ===
    if email_configured: # Usa a flag definida no início
        send_completion_email(
            ...
            # Tempos Específicos (Médios e Totais dos livros OK)
